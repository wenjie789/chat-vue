    ...
    <div>
        <div class="compose-wrap" v-if="store.state.userInfo.id > 0">
            ...
            <div class="link-wrap" v-if="showLinkSet">
                <n-dynamic-input
                    v-model:value="links"
                    placeholder="請輸入以http(s)://開頭的鏈接"
                    :min="0"
                    :max="3"
                >
                    <template #create-button-default> 創建鏈接 </template>
                </n-dynamic-input>
            </div>
        </div>
        ...
        <div class="compose-wrap" v-else>
            <div class="login-wrap">
                <span class="login-banner"> 登錄後，精彩更多</span>
            </div>
...
const visibilities = [
    {value: VisibilityEnum.PUBLIC, label: "公開"}
    , {value: VisibilityEnum.PRIVATE, label: "私密"}
    , {value: VisibilityEnum.FRIEND, label: "好友可見"}
];
...
const beforeUpload = async (data: any) => {
    ...
    if (
        ...
        window.$message.warning('圖片僅允許 png/jpg/gif 格式');
        return false;
    }

    if (uploadType.value === 'image' && data.file.file?.size > 10485760) {
        window.$message.warning('圖片大小不能超過10MB');
        return false;
    }
    ...
    if (
        ...
        window.$message.warning('視頻僅允許 mp4/mov 格式');
        return false;
    }
    ...
    if (
        ...
        window.$message.warning('視頻大小不能超過100MB');
        return false;
    }
    // 附件类型校验
    if (
        ...
        window.$message.warning('附件僅允許 zip 格式');
        return false;
    }

    if (uploadType.value === 'attachment' && data.file.file?.size > 104857600) {
        window.$message.warning('附件大小不能超過100MB');
        return false;
    }
