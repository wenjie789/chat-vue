    ...
    <div>
        ...
        <n-list class="main-content-wrap profile-wrap" bordered>
            ...
            <n-tabs class="profile-tabs-wrap" type="line" animated :value="pageType" @update:value="changeTab">
                <n-tab-pane name="post" tab="已發布"> </n-tab-pane>
                <n-tab-pane name="comment" tab="評論"> </n-tab-pane>
                <n-tab-pane name="highlight" tab="亮點"> </n-tab-pane>
                <n-tab-pane name="media" tab="圖文"> </n-tab-pane>
                <n-tab-pane name="star" tab="喜歡"> </n-tab-pane>
            </n-tabs>
            ...
            <div v-else>
                <div class="empty-wrap" v-if="list.length === 0">
                    <n-empty size="large" description="暫無數據" />
                </div>
        ...
        <n-space v-if="totalPage > 0" justify="center">
            <InfiniteLoading class="load-more" :slots="{ complete: '没有更多了', error: '載入出錯' }" @infinite="nextPage()">
                <template #spinner>
                    <div class="load-more-wrap">
                        <n-spin :size="14" v-if="!noMore" />
                        <span class="load-more-spinner">{{ noMore ? '没有更多了' : '載入更多' }}</span>
                    </div>
                </template>
            </InfiniteLoading>
        </n-space>
...
const openDeleteFriend = () => {
    dialog.warning({
        title: '删除好友',
        content: '將好友 “' + user.nickname + '” 刪除，將同時刪除 點讚/收藏 清單中關於該朋友的 「好友可見」 推文',
        positiveText: '確定',
        negativeText: '取消',
        onPositiveClick: () => {
            userLoading.value = true;
...
const handleFollowUser = () => {
    dialog.success({
        title: '提示',
        content:
            '確定' + (user.is_following ? '取消關注' : '關注') + '該使用者嗎？',
        positiveText: '確定',
        negativeText: '取消',
        onPositiveClick: () => {
            userLoading.value = true;
