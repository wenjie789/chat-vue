        ...
        <n-list class="main-content-wrap" bordered>
            ...
            <div class="comment-opts-wrap" v-if="post.id > 0">
                <n-tabs type="bar" justify-content="end" size="small" animated @update:value="commentTab">
                    <template #prefix>
                        <span class="comment-title-item">評論</span>
                    </template>
                    <n-tab-pane name="default" tab="默認" />
            ...
            <div v-if="post.id > 0">
                ...
                <div v-else>
                    <div class="empty-wrap" v-if="comments.length === 0">
                        <n-empty size="large" description="暫無評論，快來搶沙發" />
                    </div>
            ...
            <n-space v-if="comments.length >= pageSize" justify="center">
                <InfiniteLoading class="load-more" :slots="{complete: '沒有更多數據了', error: '載入出錯'}" @infinite="loadComments">
                    <template #spinner>
                        ...
                        <span v-if="defaultCommentsSort && !defaultNoMore" class="load-more-spinner" >載入評論</span>
                        <span v-if="!defaultCommentsSort && !newestNoMore" class="load-more-spinner" >載入評論</span>
                    </template>
                </InfiniteLoading>
            </n-space>
        </n-list>
